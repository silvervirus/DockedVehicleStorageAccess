...
namespace DockedVehicleStorageAccess.Patches
{
	...
	class VehicleDockingBay_Start_Patch
	{
        ...
        private static readonly string ID = System.Guid.NewGuid().ToString();
        ...
        private static IEnumerator CreateConsole(VehicleDockingBay __instance)
        {
            ...
            var guiscreen = console.transform.Find("GUIScreen");
            Debug.Log("GUIScreen found: " + (guiscreen != null));

            var canvas = guiscreen?.GetComponentInChildren<Canvas>(true);
            if (canvas != null)
            {
                canvas.gameObject.SetActive(true);
                canvas.enabled = true;
                Debug.Log("Canvas component found and activated.");
            }
            else
            {
                Debug.LogError("Canvas component not found under GUIScreen.");
            }
            console.SetActive(true);
        }
